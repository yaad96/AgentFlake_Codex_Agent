...
# share the codex_outputs directory.
#
...
if [[ ! -d "$STEPS_OUT_DIR" ]]; then
  echo "[archive] no codex_outputs at $STEPS_OUT_DIR - nothing to archive" >&2
  exit 0
fi
...
mkdir -p "$run_dir/codex_outputs"

# Copy orchestrator outputs only; skip stale Codex driver leftovers.
for f in "$STEPS_OUT_DIR"/*; do
  [[ -e "$f" ]] || continue
  case "$(basename "$f")" in
    trial.ndjson|thinking.txt|tool_calls.jsonl|prompt_system.txt|prompt_user.txt|codex.stderr|usage.json)
      continue ;;
  esac
  cp -pR "$f" "$run_dir/codex_outputs/"
done
